...
float pid_control(float except,float current,float delta_T)
{
  ...
  /**----------------------------Inside Control----------------------------**/
	...
	//if(fabs(err_ins)<300)err_ins=0;				//死区
  ...
  /**----------------------------------------------------------------------**/
	...
	pid_out	= out_ins;
	if((pid_out<22000)&&(pid_out>1500))pid_out=16000;				//如果输出值在1500~22000，则恒定输出16000，防啸叫
	if((pid_out>-22000)&&(pid_out<-1500))pid_out=-16000;		//如果输出值在-1500~-22000，则恒定输出-16000，防啸叫
	if(fabs(pid_out)<=1500)pid_out=0;												//如果输出值在1500以下，输出为0，防振荡
	return pid_out;
}
